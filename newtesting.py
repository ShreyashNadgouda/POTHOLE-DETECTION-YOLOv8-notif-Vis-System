...
# Flask app for real-time visualization
from flask import Flask, render_template, request, jsonify

# Initialize Flask app
app = Flask(__name__)

# Load the CSV file to simulate real-time pothole detection
data = pd.read_csv('/Users/shreeshnadgouda/Desktop/Pothole_Detection/cleaned_lat_long_data.csv')

# Extract coordinates where potholes are detected (PCI == 7)
coordinates = data[data['PCI'] == 7][['Latitude', 'Longitude']].values

# Global variables to store detected pothole coordinates and current index
pothole_coords = []
current_index = 0

# Function to simulate real-time detection by iterating through CSV data
def simulate_real_time_detection():
    global current_index, pothole_coords
    while current_index < len(data):
        row = data.iloc[current_index]
        if row['PCI'] == 7:
            pothole_coords.append((row['Latitude'], row['Longitude']))
        current_index += 1
        time.sleep(1)  # Simulate real-time by adding a delay of 1 second per row

# Start the simulation in a separate thread to avoid blocking the Flask server
thread = threading.Thread(target=simulate_real_time_detection)
thread.daemon = True
thread.start()
...
# Extract GPS metadata from the selected video
